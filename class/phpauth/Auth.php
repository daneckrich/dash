...
class Auth
{
    ...
    public function login($email, $password, $remember = 0, $captcha = NULL)
    {
        ...
        if ($sessiondata == false) {
            $return['message'] = $this->lang["system_error"] . " SESSION #01";
            return $return;
        }
    ...
    */
    ...
    public function register($email, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = NULL)
    {
        ...
        if ($zxcvbn->passwordStrength($password)['score'] < intval($this->config->password_min_score)) {
            $return['message'] = $this->lang['password_weak'].' SCORE: '.$zxcvbn->passwordStrength($password)['score'] . ' ' . intval($this->config->password_min_score);
    ...
    */
    ...
    protected function addUser($email, $password, $params = array(), &$sendmail)
    {
        ...
        if (!$query->execute()) {
            $return['message'] = $this->lang["system_error"] . " #03 " . "INSERT INTO {$this->config->table_users} (isactive) VALUES (0)";
            return $return;
        }
    ...
    */
    ...
    public function getSessionHash(){
        return isset($_SESSION[$this->config->cookie_name]) ? $_SESSION[$this->config->cookie_name] : false;
    }
    ...
    */
	...
	public function extendSession($hash){

		if ($hash === false) {
           return false;
        }
		if(isset($hash)){
			$query = $this->dbh->prepare("UPDATE {$this->config->table_sessions} set expiredate = CURRENT_TIMESTAMP + INTERVAL '30 MINUTE' WHERE hash = ?");
			$query->execute(array($hash));
		}
		
		// $data['expire'] = strtotime($this->config->cookie_forget);
		//setcookie($this->config->cookie_name, $hash, $data['expire'], $this->config->cookie_path, $this->config->cookie_domain, $this->config->cookie_secure, $this->config->cookie_http);
       // $_COOKIE[$this->config->cookie_name] = $hash;
	   $_SESSION[$this->config->cookie_name] = $hash;
	}
	
	public function checkLogin(){
	
		$loginurl = "/account/?destination=" . urlencode($_SERVER['REQUEST_URI']);
	...
	public function getSessionInfo($uid){
	
		$query = $this->dbh->prepare("SELECT * from {$this->config->table_sessions} WHERE uid = ?");
		$query->execute(array($uid));
